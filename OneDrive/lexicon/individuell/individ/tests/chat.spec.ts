...
test.describe("Study Buddy chatbot", () => {
  ...
  test("scrolls to the newest assistant response", async ({ page }) => {
    ...
    expect(responsePosition.top).toBeGreaterThanOrEqual(-10);
  ...
  const details = page.locator("details.about-study-buddy");
  const summary = details.locator("summary");
  const explanation = details.locator("p");

  await expect(details).toBeVisible();
  await expect(summary).toHaveText("What is Study Buddy?");
  ...
  await expect(explanation).toContainText("No data will be stored.");
});
